 ...
 * 
 * This filter then gets the bean named milton.http.manager and uses that
 * for milton processing.
 * 
 * Requests with a path which begins
 * with one of the exclude paths will not be processed by milton. Instead, for
 * these requests, the filter chain will be invoked so the request can be serviced by JSP
 * or a servlet, etc
 * 
 ...
 * 
 * This allows non-milton resources to be accessed, while still mapping all
 * urls to milton
 *
 * @author bradm
 */
public class SpringMiltonFilter implements javax.servlet.Filter {
	 ...
	 * from milton. Instead, this filter will allow filter processing to continue
	 * so they will be served by JSP or a servlet
	 */
