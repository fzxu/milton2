...
public class SchedulingXmlHelper {
    ...
    public String generateXml(List<SchedulingResponseItem> respItems) throws UnsupportedEncodingException {
        ...
        for(SchedulingResponseItem resp : respItems) {
            ...
            elRecip.begin("D","href").writeText(resp.getRecipient()).close();
            elRecip.close();
            StatusResponse stat = resp.getStatus();
            elRecip.begin(CALDAV_PREFIX,"request-status").writeText(stat.code + ";" + stat.description).close();
            elRecip.begin(CALDAV_PREFIX,"href").writeText(resp.getiCalText() ).close();
            elResp.close();
        }
